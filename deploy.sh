...
# Configuration
REGISTRY=${REGISTRY:-"543148812499.dkr.ecr.us-east-2.amazonaws.com/supbot"}
TAG=${TAG:-"latest"}
NAMESPACE="bot-infra"
...
build_images() {
    ...
    docker build -t ${REGISTRY}/slack-app:${TAG} -f slack-app/Dockerfile .
    
    echo "Building sync job image..."
    #docker build -t ${REGISTRY}/sync-job:${TAG} sync-job/
    docker build -t ${REGISTRY}/sync-job:${TAG} -f sync-job/Dockerfile .
...
push_images() {
    ...
    sudo docker push ${REGISTRY}/slack-app:${TAG}
    
    echo "Pushing ${REGISTRY}/sync-job:${TAG}..."
    sudo docker push ${REGISTRY}/sync-job:${TAG}
...
deploy_kubernetes() {
    ...
    sed "s|543148812499.dkr.ecr.us-east-2.amazonaws.com/supbot/slack-app:latest|${REGISTRY}/slack-app:${TAG}|g" kubernetes/slack-app/deployment.yaml > /tmp/slack-app-deployment.yaml
    ...
    sed "s|543148812499.dkr.ecr.us-east-2.amazonaws.com/supbot/sync-job:latest|${REGISTRY}/sync-job:${TAG}|g" kubernetes/sync-job/cronjob.yaml > /tmp/sync-job-cronjob.yaml
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            -h|--help)
                ...
                echo "  --registry       Docker registry (default: 543148812499.dkr.ecr.us-east-2.amazonaws.com/supbot)"
